...
- **Intrinsic dimension estimation**: Levina-Bickel kNN MLE, implemented in `src/nase/estimators/intrinsic_dimension.py`. Not integrated into the main pipeline yet.
...
| Intrinsic dim | `src/nase/estimators/intrinsic_dimension.py` | Levina-Bickel MLE estimator (standalone, not yet wired into runner) |
...
**TODO.** The Levina-Bickel estimator is implemented (`src/nase/estimators/intrinsic_dimension.py`) and has unit tests, but it is not yet wired into the experiment runner or any config. A future config could use the estimated dimension as a soft prior on `k*`.

### Phase 3: Real data

**TODO.** No real-data configs or loaders exist yet. A natural next step would be single-cell RNA-seq or image-patch datasets where ground-truth manifold structure is approximately known.
...
├── configs/                      # experiment YAML configs (9 files)
...
- Integrate the Levina-Bickel intrinsic dimension estimator as a soft prior on `k*` (Phase 2).
- Add real-data experiments with single-cell or image-patch datasets (Phase 3).
- Tune the epsilon grid per manifold (or make it adaptive).
...
- Developed a bandwidth-stability truncation criterion for diffusion maps that adapts the embedding dimension to noise level, validated on 5 synthetic manifolds across 50+ seeded experiment runs.
...
- Maintained a 12-file test suite with pytest covering data generation, graph construction, eigensolvers, cutoff methods, metrics, plotting, and end-to-end runner smoke tests.
