...
% ============================================================================
\begin{abstract}
Diffusion maps and Laplacian eigenmaps embed high-dimensional point clouds into low-dimensional representations by retaining leading eigenvectors of a graph operator. The number of retained eigenvectors, i.e.\ the spectral truncation point, matters: too few discard structure, too many embed noise. The noisy-Laplacian theory predicts that the transition between signal and noise eigenvectors occurs at an index scaling as $C/r^2$, where $r$ is the noise standard deviation and $C$ is a geometry-dependent constant. However, neither $C$ nor $r$ is available in practice.

We implement and evaluate three data-driven truncation strategies on five synthetic manifolds across 120 seeded experiment runs: (i)~a \emph{bandwidth-stability} rule that identifies signal modes by their invariance to kernel bandwidth perturbations, (ii)~a \emph{noise-amplitude-based} cutoff that estimates $r$ from nearest-neighbor distances and applies $k^* = \lfloor C/r^2 \rfloor$, and (iii)~the standard \emph{eigengap} heuristic. Bandwidth stability produces noise-adaptive, seed-consistent cutoffs on manifolds with uniform curvature (on the circle, the cutoff decreases from $k{=}8$ to $k{=}4$ as noise increases eight-fold, with zero seed variance) and outperforms the eigengap by $+3.5$ percentage points in trustworthiness on the S-curve. The method fails on the Swiss roll due to eigenvector reordering across bandwidths. The kNN noise estimator mixes manifold geometry with noise, underestimating $r$ on the circle ($0.3$--$0.7\times$ true) and overestimating on the Swiss roll ($2$--$7\times$), which aligns with the known difficulty of noise estimation from nearest-neighbor distances. We validate estimator accuracy against the empirical noise standard deviation computed directly from the known noise vectors, providing ground truth tighter than the configured noise parameter.
\end{abstract}
...
\end{equation}
This gives a tighter ground truth than the configured noise parameter $r$: while $r$ describes the distribution from which noise was drawn, $r_{\text{empirical}}$ captures the actual noise realization in each run. For our sample sizes ($n = 450$, $D = 3$--$5$), $r_{\text{empirical}} / r_{\text{configured}}$ falls in $[0.976, 1.009]$, which verifies data generation accuracy and gives a precise benchmark for evaluating the kNN estimators.
...
\end{itemize}
Trustworthiness tends to be high even for large $k$ (adding dimensions rarely creates false neighbors), while continuity is more sensitive to the embedding dimension. Geodesic consistency measures whether the embedding preserves global manifold geometry, not just local neighborhoods.
...
The circle--sphere noise sweep is the clearest case for bandwidth stability. On the circle ($n = 450$, $D = 3$), the cutoff decreases monotonically with noise and is perfectly consistent across seeds (Table~\ref{tab:circle_results}).
...
The stability profile (Figure~\ref{fig:stability_circle}) reveals a sharp boundary between signal and noise modes. At $r = 0.02$, modes 1--8 have stability scores above 0.999, while mode~9 drops sharply. At $r = 0.16$, the boundary shifts to mode~4, with mode~5 dropping to 0.847. This matches the behavior predicted by the noisy-Laplacian theory: more noise corrupts more modes, and the stability metric picks up this boundary.

The eigengap always selects $k = 2$ (the gap after the cosine--sine pair), regardless of noise level. It cannot adapt because the spectral gap structure of the circle does not change with noise: the leading eigenvalues stay separated, and only the eigenvectors beyond the gap change character.

Trustworthiness stays above 0.98 at all noise levels, so the bandwidth-stability cutoff does not include overtly noisy modes. Continuity drops from 0.885 to 0.453 as noise increases, which is expected since noise degrades the embedding's fidelity to the original neighbor structure. Geodesic consistency holds around 0.75--0.78, so the embedding preserves global geometry even as local quality degrades.
...
On the sphere ($n = 450$, $D = 5$), the stability cutoff again decreases with noise: $k = 12 \to 10.7 \to 8$ (Table~\ref{tab:sphere_results}). At medium noise, one mode near the stability boundary causes slight seed variance ($k_{\text{stab}} = 10.7 \pm 1.2$): across the three seeds, the method selects $k = 10, 12, 10$. This is expected near the threshold, where the binary accept/reject decision is sensitive to the specific noise realization. The low and high noise conditions have zero seed variance.

Geodesic consistency on the sphere (${\sim}0.44$) is lower than on the circle (${\sim}0.78$), likely because geodesic distances on the sphere are harder to approximate from low-dimensional embeddings.
...
The S-curve experiment was designed per the professor's feedback to test whether the stability method finds the correct cutoff when the spectral gap is ambiguous. The S-curve has a gradual eigenvalue decay with no pronounced gap. As shown in Table~\ref{tab:scurve_results}, the eigengap picks $k = 1$, while bandwidth stability retains 13 modes with $+3.5$ percentage points in trustworthiness and $+7.9$~pp in continuity. Both methods are seed-consistent on this manifold.

That said, $k = 13$ seems high for a 2-dimensional manifold. Inspecting the stability profile, there is a dip at mode~10 (score 0.84) that recovers at mode~13 (score 0.91). The current rule (take the \emph{largest} $k$ above threshold) jumps over this dip. A ``first-drop'' rule that stops at the first mode below threshold would give $k = 9$, which may be more appropriate. This non-monotonic behavior suggests there is spectral structure that a fixed threshold does not fully capture.
...
The Swiss roll is our main negative result. Across all experiments (single runs, noise sweeps with $r = 0.03, 0.08, 0.16$, and bandwidth sweeps with narrow and wide grids), the stability method selects $k = 1$. Even with a narrow bandwidth grid ($2.2\times$ ratio from minimum to maximum $\varepsilon$), stability scores fail to reach $\tau = 0.9$ for any mode above $k = 1$.

The failure mechanism is worth examining. The Swiss roll has non-uniform curvature (a spiral), and small bandwidth perturbations cause eigenvectors to \emph{rotate} or \emph{reorder} within their eigenspace. This destroys per-vector alignment even though the \emph{subspace} spanned by modes $1$--$k$ may be stable. On the circle and sphere, the eigenvectors correspond to Fourier modes and spherical harmonics, which are globally defined and robust to bandwidth changes. On the Swiss roll, modes~2 and~3 can mix under small perturbations, and our per-vector dot-product metric reads this as instability.

The eigengap also fails on the Swiss roll: across the noise sweep, $k_{\text{gap}}$ ranges from 1 to 3 across seeds, and geodesic consistency is negative (${\sim}{-}1.97$), meaning the embedding's distance structure is \emph{anti-correlated} with the manifold's geodesic structure at these parameter settings.
...
The torus ($d = 2$, $D = 5$) is less clean than the circle or sphere: $k_{\text{stab}} = 8.7, 10.0, 8.7$ across noise levels with standard deviations of $2.1$--$2.9$. The torus has two independent circular dimensions, producing a richer eigenvalue spectrum where multiple modes sit near the threshold boundary at different noise levels. Trustworthiness stays high (${\sim}0.96$) and geodesic consistency (${\sim}0.78$) is good, so the embedding captures the manifold structure even though the cutoff itself is less stable.

\subsection{Sphere in Higher Ambient Dimension: When the Eigengap Suffices}

On the sphere embedded in $\mathbb{R}^6$ with $r = 0.14$, the eigengap is perfectly stable ($k = 3$, zero variance across seeds) because the sphere's spectral gap after the first three spherical harmonics is pronounced even at this noise level. The stability method gives $k = 8$--$9$ with slight variance. This shows the eigengap is not universally bad; it works well when the spectrum has a genuine cliff. The trouble is manifolds where the cliff does not exist (S-curve) or does not shift with noise (circle).

\subsection{Threshold Sensitivity}
\label{sec:threshold_sensitivity}

We tested four stability thresholds $\tau \in \{0.80, 0.85, 0.90, 0.95\}$ on the circle at $r = 0.08$ (Figure~\ref{fig:threshold_sensitivity}). The cutoff is $k = 6$ for $\tau \in \{0.85, 0.90, 0.95\}$ and $k = 6.33$ at $\tau = 0.80$. This insensitivity arises because stability scores drop sharply (from ${>}0.999$ to ${\sim}0.745$) at the signal--noise boundary, making the exact threshold irrelevant on this manifold. Only at $\tau = 0.80$ does one additional mode occasionally pass. This is encouraging because it means the method is not very sensitive to the threshold hyperparameter, at least on manifolds with clean spectral structure.

On the Swiss roll, $\tau$ matters more: lowering it below 0.9 might let some modes through but would risk including noise. The threshold's effect is manifold-dependent, tied to how sharp the stability drop-off is.
...
To understand why the oracle picks such low $k$ values ($k = 1$--$2$ across all conditions), we examined the subspace distance $d_{\text{sub}}(k)$ between clean and noisy eigenspaces as a function of $k$ (Figure~\ref{fig:oracle_profiles}). For both the circle and sphere, $d_{\text{sub}}$ is minimized at $k = 1$--$2$ and grows with $k$. Adding more noisy eigenvectors increases the mismatch with the clean eigenspace.

The point here is that the oracle minimizes \emph{subspace distance from the clean eigenspace}, not embedding quality. A single well-aligned eigenvector gives a low subspace distance but yields a 1D embedding that discards too much structure for practical use. When we evaluate the oracle's chosen $k$ using trustworthiness and continuity (Table~\ref{tab:quality_at_k}), the oracle produces worse embeddings than either the eigengap or bandwidth stability. The two criteria are measuring different things.
...
\paragraph{Empirical $r$ vs.\ configured $r$.} Across all 120 runs, $r_{\text{empirical}} / r_{\text{configured}}$ falls in $[0.976, 1.009]$, so the finite-sample noise realization closely matches the target distribution (Figure~\ref{fig:empirical_r_comparison}, left). This means the entire gap between $r_{\text{empirical}}$ and $\hat{r}_{\text{kNN}}$ is due to estimator bias, not data-generation artifacts.
...
The estimation biases can be understood geometrically:

\begin{enumerate}
    \item \textbf{Circle ($D = 3$, $d = 1$):} On a 1D curve in 3D, nearest-neighbor distances are dominated by the manifold arc length, not noise. The noise contribution to the total NN distance is small, so dividing by $2D$ in Eq.~\eqref{eq:simple_estimator} overcompensates. The estimator underestimates $r$ by $30$--$70\%$, and the bias gets worse at higher noise because the arc-length contribution stays roughly constant while the noise grows.

    \item \textbf{Sphere ($D = 5$, $d = 2$):} At low noise ($r = 0.02$), pairwise distances on the sphere surface are large even without noise, causing overestimation ($2.1\times$). As noise increases, the noise term becomes relatively larger, and the estimator shifts to underestimation ($0.59\times$ at $r = 0.16$). The crossover is near $r \approx 0.05$.

    \item \textbf{Swiss roll ($D = 3$, $d = 2$):} The spiral geometry means that nearest-neighbor distances along the manifold far exceed the noise contribution at all tested noise levels. The estimator returns $\hat{r} \approx 0.21$ whether $r_{\text{true}} = 0.03$ or $r_{\text{true}} = 0.16$, so it carries no useful information about the noise level.

    \item \textbf{Torus ($D = 5$, $d = 2$):} Shows the same crossover as the sphere: overestimation at low noise ($3.5\times$), shifting to underestimation at high noise ($0.77\times$). The crossover is near $r \approx 0.08$.
\end{enumerate}
...
On the circle with estimated $r$, the cutoff decreases from 12 to 5 as noise increases from 0.02 to 0.16, which is qualitatively correct. What happens is that the estimator's underestimation of $r$ ($\hat{r} < r_{\text{true}}$) means $C/\hat{r}^2 > C/r_{\text{true}}^2$, which partially compensates for the small $C$. This accidental cancellation produces reasonable cutoffs, but it is not a reliable mechanism since it depends on the specific magnitude and direction of two independent biases.
...
On the sphere with estimated $r$, the cutoff decreases from 9.7 to 2.0, which is reasonable but much lower than bandwidth stability's $12 \to 8$. At $r = 0.12$ and $r = 0.16$, $k = 2$ is likely too few for a useful embedding.
...
The central question of this project was whether bandwidth stability can work as a reliable, noise-adaptive truncation criterion for diffusion maps. Our experiments give a mixed answer: \emph{yes, on manifolds with uniform curvature, and no, on manifolds where eigenvectors reorder across bandwidths.}

The method's main strength is that it adapts to the noise level without requiring knowledge of $r$ or $C$. On the circle, the stability cutoff tracks the noisy-Laplacian theory's prediction closely: as noise increases $8\times$ (from $r = 0.02$ to $r = 0.16$), the cutoff decreases from $k = 8$ to $k = 4$ with \emph{zero variance across seeds}. The eigengap cannot do this; it always picks $k = 2$ regardless of noise, leaving information on the table at low noise levels. On the S-curve, where the eigengap picks $k = 1$ due to gradual spectral decay, bandwidth stability retains 13 modes and achieves better embedding quality ($+3.5$~pp trustworthiness, $+7.9$~pp continuity). Both methods are seed-consistent on the S-curve, which suggests the eigengap's instability on the Swiss roll is not a general sensitivity problem but a structural one: it depends on whether the spectrum has the sharp-gap structure the eigengap assumes.

The torus results are in between: the cutoff adapts to noise (decreasing from $8.7$ to $8.7$ with a non-monotonic peak at $10.0$ for medium noise) but with higher seed variance ($2.1$--$2.9$). The torus's two independent circular dimensions produce a richer spectrum than the circle or sphere, and the stability landscape is correspondingly more complex. Even so, trustworthiness remains above 0.96 and geodesic consistency above 0.77 across all conditions.

\subsection{The Swiss Roll Failure: Per-Vector vs.\ Subspace Alignment}

The Swiss roll failure is the most informative result of the project. The method measures stability by aligning \emph{individual} eigenvectors: $|\langle \psi_k^{(a)}, \psi_k^{(b)} \rangle|$. On the Swiss roll, even a small bandwidth change can cause eigenvectors to rotate within their eigenspace (e.g., modes 2 and 3 mixing) without the underlying signal quality degrading. This looks like instability to our per-vector metric even though the \emph{subspace} $\mathrm{span}\{\psi_1, \ldots, \psi_k\}$ may be stable.

The fix is straightforward in principle: replace per-vector dot products with a subspace distance metric like the maximum principal angle between $\mathrm{span}\{\psi_1, \ldots, \psi_k\}^{(a)}$ and $\mathrm{span}\{\psi_1, \ldots, \psi_k\}^{(b)}$. This would be invariant to rotations within the subspace and would only flag genuinely different subspaces. We have the principal-angle computation in our codebase and a placeholder for this approach (\texttt{adjacent\_subspace\_stability\_stub} in \texttt{bandwidth\_stability.py}), but did not complete the integration. This is the highest-priority extension.
...
\subsection{The Difficulty of Noise Estimation from kNN Distances}

Our kNN noise estimation experiments provide empirical evidence for the difficulty anticipated in the course feedback. The core issue is Eq.~\eqref{eq:expected_distance}: nearest-neighbor distances contain \emph{both} noise and manifold contributions, and separating the two requires knowing the manifold, which is what we are trying to learn in the first place. This circular dependency plays out differently across manifold geometries:

On the circle ($d = 1$, $D = 3$), the 1D arc is densely sampled (450 points on a unit circle), so nearest-neighbor distances along the manifold are small but not negligible compared to the noise. The estimator divides total NN distance by $2D = 6$, attributing too much to noise in the denominator. The result is systematic underestimation that gets worse as noise increases, because the manifold contribution stays roughly constant while the noise grows, but the manifold contribution is never truly negligible at these sample sizes.

On the sphere and torus ($D = 5$), the higher ambient dimension means the $2D$ divisor is larger, and the manifold surface distances at the nearest-neighbor scale are also large. At low noise, the manifold distances dominate and the estimator overestimates $r$. At high noise, the noise term becomes the larger component, but the formula still attributes some manifold distance to noise, leading to underestimation. Where the crossover happens depends on the specific manifold geometry.

The Swiss roll is the worst case: its spiral structure means that even nearest neighbors on the manifold surface are separated by large distances due to the spiral windings. The estimator returns $\hat{r} \approx 0.21$ whether $r_{\text{true}} = 0.03$ or $0.16$, because the manifold contribution overwhelms the noise at all tested scales.

The empirical noise amplitude analysis (Section~\ref{sec:empirical_r_method}) supports these observations. By computing $r_{\text{empirical}} = \mathrm{std}(X_{\text{noisy}} - X_{\text{clean}})$ directly from the known noise vectors, we verify that $r_{\text{empirical}} / r_{\text{configured}} \in [0.976, 1.009]$. The data generation is accurate, so the entire estimation gap lies between $r_{\text{empirical}}$ and $\hat{r}_{\text{kNN}}$. This rules out the possibility that the bias is an artifact of how the data was generated.

\subsection{Theory vs.\ Practice for the \texorpdfstring{$C/r^2$}{C/r-squared} Formula}

The theoretical formula $k^* = C/r^2$ is clean but faces three practical obstacles that compound:

\begin{enumerate}
    \item \textbf{$C$ is tiny and hard to calibrate.} The empirical $C$ values (${\sim}0.015$) are very small, meaning the formula transitions from ``keep maximum modes'' to ``keep 1 mode'' over a narrow noise range. This steep dependence on $r$ makes the formula fragile: small errors in $r$ or $C$ cause large changes in $k^*$. On top of this, $C$ was calibrated from oracle data (which itself is very conservative); in a real-world setting, $C$ would need to be estimated or transferred from similar manifolds.
...
\paragraph{Per-vector alignment.} The Swiss roll failure is the main algorithmic limitation. Replacing per-vector dot products with subspace-based stability (principal angles between the span of leading $k$ eigenvectors at different bandwidths) is the highest-priority extension and can be done with existing code.
...
\paragraph{Scale and dimensionality.} All experiments use $n \leq 500$ and $D \leq 6$. Larger samples would sharpen eigenvalue separation and might reveal the $r^{-2}$ scaling more clearly. Higher ambient dimensions could change the effective range of the $\varepsilon$-grid and the behavior of the kNN noise estimator.

\paragraph{Real data.} We tested only on synthetic manifolds with isotropic Gaussian noise. Real-world data brings unknown intrinsic dimension, non-Gaussian and heteroscedastic noise, varying sampling density, and no oracle access. Good candidates for real-data validation would be single-cell RNA-seq (developmental trajectories), image patches (Klein bottle manifold), and MNIST digit classes.

\paragraph{Improved noise estimation.} Other approaches, such as random projection into low-dimensional subspaces before estimating $r$, or iterative refinement that alternates between manifold recovery and noise estimation, could reduce the bias of the kNN estimator and make the $r$-based formula more practical.
...
\begin{enumerate}
    ...
    \item \textbf{kNN noise estimation is biased by manifold geometry.} The estimator underestimates $r$ on the circle ($0.3$--$0.7\times$), overestimates on the Swiss roll ($2$--$7\times$), and shows a dimension-dependent crossover on the sphere and torus. These biases are validated against the empirical noise amplitude computed directly from the known noise vectors.

    \item \textbf{The formula $k^* = C/r^2$ is fragile in practice.} The empirically calibrated $C$ is very small (${\sim}0.015$), and the composition of biased $C$ and biased $\hat{r}$ can accidentally produce reasonable cutoffs but is not reliable as a general method.
\end{enumerate}

The project shows that bandwidth stability is a viable practical alternative to direct noise estimation for spectral truncation. It also provides empirical evidence for why noise estimation from nearest-neighbor distances is hard in practice, which aligns with the theoretical predictions that motivated this alternative approach.

% ============================================================================
\begin{ack}
This project was conducted for DSC~205 (Winter 2026) at the University of California, San Diego. We thank the course instructor for feedback on the project proposal, particularly the suggestion to use bandwidth stability as a practical alternative to noise estimation and the warning about kNN-based noise estimation being biased by manifold geometry. Both observations shaped the project's direction and are supported by the experimental results in this report.
\end{ack}
